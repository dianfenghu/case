...
class NewsController extends AllowController {
    public function index(){
    	...
    	if($Title){
    		$where="tx_news.Title like '%$Title%'";
    	}else{
    		$where='';
    	}
    	$newsList=M('news')->where($where)->order('tx_news.Sequence desc,tx_news.Nid desc')->join('tx_news_class on tx_news_class.Cid=tx_news.Class')->field("*,tx_news.AddTime as AddTime,tx_news_class.AddTime as CaddTime")->select();
    ...
    public function delNewsClass(){
        $Cid=$_GET['Cid'];
        $count=M('news')->where("Class={$_GET['Cid']}")->count();
        if($count>0){
            $this->error('请先删除类别下新闻');
            exit;
        }
    ...
    public function Sequence(){
        $row=A('menu')->_Sequence('news_class','Cid',$_POST);
        if($row){
            $this->success('排序成功');
        }else{
            $this->error('排序失败');
    ...
    public function NewsSequence(){
        $row=A('menu')->_Sequence('news','Nid',$_POST);
        if($row){
            $this->success('排序成功');
        }else{
            $this->error('排序失败');
